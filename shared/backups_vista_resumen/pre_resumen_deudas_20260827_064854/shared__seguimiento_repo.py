...
_COLUMNAS_PROYECCION = ["MZ", "LT", "MES", "DEUDA", "PAGO", "DECLARADO", "AJUSTE", "SALDO"]
        ...
        for mes in meses:
            ...
            datos = {
                nombre: {"MZ": mz, "LT": lt, "MES": mes, "DEUDA": 0.0,
                         "PAGO": 0.0, "DECLARADO": 0.0, "AJUSTE": 0.0, "SALDO": 0.0}
                for nombre in filas
            }
            ...
            for _, ev in eventos.iterrows():
                ...
                columna = "PAGO" if clase in CLASES_SUMAN_CAJA else "DECLARADO"
            ...
            pago_esperado, declarado_esperado = _partir_pago(eventos)
            totales = {
                campo: round(sum(datos[b][campo] for b in datos), 2)
                for campo in ("DEUDA", "PAGO", "DECLARADO", "SALDO")
            }
            saldo_esperado = round(deuda_esperada - pago_esperado - declarado_esperado, 2)
            esperados = {"DEUDA": deuda_esperada, "PAGO": round(pago_esperado, 2),
                         "DECLARADO": round(declarado_esperado, 2), "SALDO": saldo_esperado}
