...
def test_vista_provisional_no_muta_ledger(tmp_path, monkeypatch):
    ...
    assert wb.sheetnames[:5] == [
        "PROVISIONAL", "MES_ANTERIOR", "MES_ACTUAL", "MANTENIMIENTO", "CORTE_RECONEXION"]
    ...
    assert ws_anterior["D3"].value == 20 and ws_anterior["E3"].value == 20
    ws_actual = wb["MES_ACTUAL"]
    assert ws_actual["D3"].value == 13 and ws_actual["H3"].value == 13
    ws_mant = wb["MANTENIMIENTO"]
    assert ws_mant["D3"].value == 3 and ws_mant["H3"].value == 3
