...
import abonos_rezagados_repo as abonos_repo  # noqa: E402
...
def _cargar_ajustes_cargo(mes_ano: str) -> dict[tuple[str, str], list[tuple[str, float]]]:
    ...
    if "ESTADO" not in df.columns:
        raise RuntimeError("ajustes_cargo: falta ESTADO; migrar la fuente antes de procesarla")
    estados = df["ESTADO"].fillna("").astype(str).str.strip().str.upper()
    desconocidos = sorted(set(estados) - {"CONFIRMADO", "DESCARTADO"})
    if desconocidos:
        raise RuntimeError(f"ajustes_cargo: ESTADO invalido {desconocidos}")
    df = df[estados.eq("CONFIRMADO")].copy()
    por_predio: dict[tuple[str, str], list[tuple[str, float]]] = {}
    for _, f in df.iterrows():
        if str(f.get("MES_ANO_APLICA", "")).strip() != mes_ano:
            continue
...
def _cargar_abonos_rezagados(mes_ano: str) -> dict[tuple[str, str], tuple[float, float]]:
    ...
    if not ABONOS_REZAGADOS_PATH.exists():
        return {}
    df = abonos_repo.leer_abonos(ABONOS_REZAGADOS_PATH)
    df = df[df["MES_ANO_APLICA"].astype(str).str.strip().str[:7] == mes_ano].copy()
    dirigidos = df[df["MODO_APLICACION"].astype(str).str.strip().str.upper() == "DIRIGIDO"]
    if not dirigidos.empty:
        raise RuntimeError(
            "Hay abonos DIRIGIDOS para este ciclo; no pueden entrar por la cascada generica. "
            "Use el dry-run y el writer append-only autorizado."
        )
    log.info(f"Abonos confirmados · {len(df)} fila(s) en cascada para {mes_ano}")
...
def _calcular(usuarios: list[dict],
    ...
    for u in usuarios:
        ...
        mes_usuario = u.get("mes_ano", "")
        if (abono_rezagado > TOL and mes_usuario
                and repo_estado.ledger_comprometido(
                    mes_usuario, ruta=ESTADO_CICLO_PATH)):
            deuda_vigente = round(sum(
                _componentes_vigentes_ledger(u, mes_usuario).values()), 2)
            if abono_rezagado > deuda_vigente + TOL:
                raise RuntimeError(
                    f"Abono rezagado excede deuda vigente: {u['mz']}-{u['lt']} "
                    f"abono={abono_rezagado:.2f} deuda={deuda_vigente:.2f}"
                )
            # El pago normal ya esta comprometido. Se suma solo para conservar
            # TOTAL_A_PAGAR bruto; el abono nuevo reduce el saldo vigente.
            total = round(deuda_vigente + pagado_normal, 2)
...
def _componentes_vigentes_ledger(r: dict, mes_ano: str) -> dict[str, float]:
    """Saldo activo para aplicar abonos agregados despues del commit mensual."""
    mz, lt = r["mz"], r["lt"]
    saldo = lambda concepto: max(round(repo.get_saldo(mz, lt, concepto, mes_ano), 2), 0.0)
    return {
        "AGUA_ANT": 0.0, "MANT_ANT": 0.0,
        "AGUA_ACT": saldo("AGUA"), "MANT_ACT": saldo("MANTENIMIENTO"),
        "CORTE_ANT": 0.0, "CORTE_ACT": saldo("CORTE_RECONEXION"),
        "CONVENIO": saldo("CONVENIO"), "ACUERDOS": saldo("ACUERDOS"),
        "MULTA": saldo("MULTA"),
    }
...
def _aplicaciones_por_fuente(r: dict, mes_ano: str) -> tuple[dict[str, dict[str, float]], dict[str, float]]:
    ...
    reapertura = (cerrado + vigente > TOL and repo_estado.ledger_comprometido(
        mes_ano, ruta=ESTADO_CICLO_PATH))
    if reapertura:
        saldos_normal = _componentes_cuenta(r, mes_ano)
        pago_normal = _aplicar_componentes(saldos_normal, normal, _ORDEN_CICLO)
        saldos = _componentes_vigentes_ledger(r, mes_ano)
