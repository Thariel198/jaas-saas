...
def _validar_confirmados(df: pd.DataFrame) -> None:
    requeridas = {
        ...
        "ESTADO", "MODO_APLICACION", "CONCEPTO_DESTINO", "APLICADO",
    }
    ...
    aplicado = _texto(df["APLICADO"]).str.upper()
    aplicados_invalidos = sorted(set(aplicado) - APLICADO_VALIDOS)
    if aplicados_invalidos:
        raise RuntimeError(f"abonos_rezagados: APLICADO invalido {aplicados_invalidos}")
