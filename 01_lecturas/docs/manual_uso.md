...
que alimenta a `02_planilla` para calcular el cobro.
...
- Genera `lecturas_planilla_YYYY-MM.xlsx` (input para 02_planilla)
...
### Paso 6 — Listo → 02_planilla

```bash
python ../02_planilla/main.py
```
...
| `outputs/lecturas_planilla_YYYY-MM.xlsx` | **PERMANECE** · uno por mes | fuente de verdad para 02_planilla |
...
- **No calcula cobros** — eso lo hace `02_planilla` a partir de `lecturas_planilla_YYYY-MM.xlsx`
- **No genera boletas** — eso es `06_boletas`
- **No corrige lecturas ya facturadas** — para eso existe `05b_override`
- **No mantiene tarifas** — vive en `shared/tarifas.xlsx` y lo lee `02_planilla`
...
`docx2pdf` requiere Microsoft Word instalado (se usa también en `06_boletas`).
