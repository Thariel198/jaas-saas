...
escribe su audit para que coincida:
  ...
  TIENE − DEBE → SOBRANTES → escribe REVERTIDO en audit
...
def _modificar_planilla(
    ...
    for p in nuevos:
        ...
        nuevo  = round(actual + config.PENALIDAD, 2)   # informativo (col 6 audit)
        aplicados.append({
            "mz":     p["mz"],
            "lt":     p["lt"],
...
def main() -> None:
    ...
    print(f"\n[4/5] Calculando penalidad (solo audit · no se escribe la planilla)...")
    if nuevos:
