...
- [ ] El módulo `4_pagos` corrió sin errores y los pagos Yape están identificados
...
La primera vez del mes, abrir una terminal en la carpeta `5_cobranza/` y ejecutar:
...
Copiar estos dos archivos a sus carpetas dentro de `5_cobranza/inputs/`:

| Archivo | Origen | Destino |
|---|---|---|
| `pagos_yape_tepago.xlsx` | `4_pagos/yape/motor_matching/outputs/` | `inputs/pagos_yape/` |
| `pagos_efectivo.xlsx` | `4_pagos/efectivo/outputs/` | `inputs/pagos_efectivo/` |
...
Abrir una terminal en la carpeta `5_cobranza/` y ejecutar:
...
```
═══════════════════════════════════════════════════════
  5_cobranza — Procesamiento de cobranza
═══════════════════════════════════════════════════════
...
═══════════════════════════════════════════════════════
  ...
  Pasar lista_corte.xlsx → 6_corte
═══════════════════════════════════════════════════════
```
...
3. ¿Los EXCESO son razonables? Montos muy grandes pueden ser errores de identificación en `4_pagos`.
...
Antes de pasar esta lista a `6_corte`, revisar manualmente si algún usuario
...
Una vez revisada, pasar a `6_corte`.
...
No se copió el archivo desde `4_pagos`. Copiar antes de ejecutar.
...
INPUTS (llenar/copiar antes de ejecutar)
  ...
  inputs/pagos_yape/pagos_yape_tepago.xlsx  ← copia de 4_pagos
  inputs/pagos_efectivo/pagos_efectivo.xlsx ← copia de 4_pagos
...
OUTPUTS (revisar después de ejecutar)
  ...
  outputs/lista_corte.xlsx                  → filtrar y pasar a 6_corte
