...
COL_TABLA = "MES_ANT"            # columna de comun.tabla_predio() — qué se aplicó
...
def _meses_esperados(hasta: str) -> list[str]:
    ...
    arranca el historial que arma comun.tabla_predio()."""
    desde = comun._ARCHIVOS_HISTORICOS[0][1]
    out, m = [], desde
    while m <= hasta:
        out.append(m)
...
def _pagos_del_ciclo(mes: str) -> pd.DataFrame:
    ...
    yape = comun._cargar_pagos_yape_crudo(mes)
    ...
    efec = comun._cargar_pagos_efectivo_crudo(mes)
...
def _excesos_no_resueltos() -> pd.DataFrame:
    """arrastre_devolucion de cada planilla_cobrado, solo filas SIN resolver.
    ...
    filas = []
    dirs = {m: comun.REPOS_CICLO_CERRADO[m] / "5_cobranza" / "outputs" for m in comun.REPOS_CICLO_CERRADO}
    ...
    for mes, carpeta in dirs.items():
        ruta = ciclo.resolver(carpeta, "planilla_cobrado", mes)
        if not ruta.exists():
            continue
        df = pd.read_excel(ruta, sheet_name="arrastre_devolucion", header=1)
        df.columns = [str(c).strip().upper() for c in df.columns]
...
def _blancos_disponibles() -> pd.DataFrame:
    ...
    for archivo, mes in comun._ARCHIVOS_HISTORICOS:
        hojas = comun._cargar_hojas_historicas(archivo)
        dfr = hojas.get("Reporte")
        if dfr is None:
            continue
        cmz = comun._col(dfr, "mz")
        cmonto, cfecha = comun._col(dfr, "monto"), comun._col(dfr, "fecha de operacion")
        corigen, cmsg = comun._col(dfr, "origen"), comun._col(dfr, "mensaje")
        if cmz is None:
            continue
...
def _debia(mz: str, lt: str, mes: str, col: str) -> float | None:
    ...
    df = comun._cargar_planilla_correcta(mes)
...
def buscar(mes: str) -> pd.DataFrame:
    ...
    historicos, eventos, mapa_raw = comun._cargar_historicos(), None, comun._cargar_mapa_raw()
    dfp = pd.read_excel(comun._planilla_cobrado_path(mes),
    ...
    def tabla_de(mz, lt):
        ...
        if (mz, lt) not in _tablas:
            ...
            _tablas[(mz, lt)] = comun.tabla_predio(mz, lt, historicos, eventos, dfp, mapa_raw)
        return _tablas[(mz, lt)]

    # Pools crudos de TODOS los ciclos con archivo disponible (los cerrados de
    # comun.REPOS_CICLO_CERRADO + el activo). Antes se cargaba solo el ciclo del
    ...
    ciclos = sorted(set(comun.REPOS_CICLO_CERRADO) | {mes})
