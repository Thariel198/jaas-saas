# libro_mayor/caja — Libro de caja (ledger de movimientos de dinero)

> Agregado del bounded context `libro_mayor/` (no es un módulo del pipeline). Ver `libro_mayor/README.md`.
>
> **Estado:** Fase 1 — spec cerrado con `libro_mayor/estado_cuenta`.
> **Origen:** reemplaza al descartado `7b_historial_pagos` (arquitectura distinta,
> eliminado — recuperable en git history). `caja_repo.py` se escribe desde cero según
> este contrato, no evoluciona el `historial_repo.py` viejo.

---

## Qué hace

Registro **append-only** de todo movimiento de dinero de la JASS, por canal y por
**balde** — lo que entra y lo que sale:

- **INGRESO** (+): plata que entró — pago de agua, aporte al tanque, deuda_directiva…
- **EGRESO** (−): plata que salió — devolución de exceso, retorno, o **gasto**
  institucional (honorario, gasto).

> `comunitario` **no es un balde** — es un mecanismo de segregación de `motor_matching`
> (1 depósito de un cobrador cubre N predios, se desglosa por lote). Ver corrección
> en la sección "Baldes y auditoría de tesorería (⑩)" del contrato, abajo.

Cada movimiento lleva `DIRECCION`, `BALDE` y `DESTINO` (ver Entidad 1 del contrato),
así la caja responde **tesorería del mes** ("cuánto entró, cuánto salió, en qué
balde") y reproduce la validación de `5b`.

Es la **fuente de verdad del dinero** — el HECHO crudo: cuánto, cuándo, por qué
canal, con qué referencia, en qué balde. **No sabe de conceptos de deuda ni de a qué
cargo se imputó** (eso es INTERPRETACIÓN → `estado_cuenta`). El `BALDE` de caja
(agua, tanque, gasto…) no es el `CONCEPTO` de deuda (consumo, multa…): un mismo
ingreso de agua es el mismo hecho aunque termine pagando la deuda de dos predios.

## Por qué existe

La pregunta "¿te pagué efectivo en mayo?" es inrespondible desde `seguimiento_pueblo`
(no guarda canal, no cubre consumo/corte, y colapsó el histórico pre-junio en un
solo número). caja es el único ledger que conserva el depósito completo, por canal,
por mes, para **cualquier** concepto y **cualquier** JASS.

## Cuándo corre

**Nunca en caliente.** Post-cierre del mes:
- **Histórico** (Oct-2025 → hoy): sembrado por backfill (motores corridos en
  repo-copia → staging → importado acá).
- **Mes en curso:** tras `7_cierre`, desde los outputs reales de `4_pagos`.

Mismo importador para los dos casos — solo cambia la carpeta fuente.
...
caja/
├── inputs/
│   └── historico/                 ← staging del backfill (pagos_efectivo_*, *_procesado)
├── importar_efectivo.py           ← INGRESOS efectivo (agua + blancos + conceptos)
├── importar_yape.py               ← INGRESOS yape (agua + blancos + conceptos)
├── importar_egresos.py            ← EGRESOS del libro PAGASTE (devolución · retorno · gasto)
├── caja_repo.py                   ← WRITER ÚNICO (se escribe desde cero según el contrato)
│                                     expone: registrar_movimiento (ingreso/egreso, con BALDE)
│                                             · identificar_abono · reasignar_abono
├── consultar.py                   ← LECTORES read-only (por balde, por predio, tesorería del mes)
└── docs/
```

> Los baldes de **ingreso por concepto** (`tanque`, `deuda_directiva`) hoy tienen
> consolidador propio en `4_pagos/` (`aportes_tanque.xlsx`, `shared/deuda_directiva.xlsx`);
> los importadores los leen y los registran con su `BALDE`. Los baldes de **egreso**
> (`honorario`/`gasto`) hoy viven sueltos en `pagos_yape_pagaste.xlsx` —
> la caja es el primer lugar que los consolida.

> **`identificar_abono` / `reasignar_abono`** son las tools que `4b_reclamos`
> invoca al resolver un reclamo de blanco o pago mal atribuido (decisión ⑨). No
> editan el abono — agregan un evento de identificación append-only que le asigna
> predio, para que el motor de `estado_cuenta` pueda aplicarlo.

## Reglas de negocio

- **Writer único:** solo `caja_repo.py` escribe el store.
- **Append-only:** un evento nunca se modifica ni se borra. Corrección = evento nuevo.
- **1 depósito = 1 abono:** un yape de S/100 que paga dos predios es **un** abono de
  S/100 — el reparto es de `estado_cuenta`, no de la caja (ver decisión ① / Hueco 1).
- **`ABONO_ID` estable y único:** determinista, lo referencia `estado_cuenta` (ver contrato).
- **Clasifica por `BALDE`, no imputa a cargos:** la caja etiqueta cada movimiento con su
  balde (agua/tanque/gasto/…) pero **no reparte por concepto de deuda ni decide a qué predio
  autoritativo va** un pago compartido. Eso es `estado_cuenta` (decisión ⑩ separa `BALDE` de `CONCEPTO`).
- **`JASS_ID` en cada evento:** multi-tenant desde el día 1 (ver decisión ⑦).
- **Idempotencia** por `ORIGEN_ARCHIVO` (re-sembrar un mes no duplica).
- **Egreso = evento de primera clase** (`DIRECCION=EGRESO`, `BALDE=devolucion/retorno/gasto/…`,
  `DEVOLUCION_ID`/`GASTO_ID` propio), no un abono negativo escondido. La caja registra
  tanto lo que entra como lo que sale para responder tesorería del mes (decisión ⑩).

## Lo que NO hace

- No reparte el ingreso por concepto de deuda (consumo/corte/multa/…) — clasifica por
  `BALDE`, que es otra cosa.
- No conoce la deuda ni calcula saldos.
- No decide prioridad de aplicación ni a qué predio se imputa (eso lo hace el **motor
  de aplicación** de `estado_cuenta`, que **referencia** los abonos de acá por `ABONO_ID`).
