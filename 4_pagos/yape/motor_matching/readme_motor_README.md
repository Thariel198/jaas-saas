...
1_lecturas
2_planilla
4_pagos/yape/motor_matching   ← estás aquí
        ↓
5_cobranza                    → consume pagos_yape_tepago.xlsx · blancos_acumulados.xlsx · devoluciones_acumulados.xlsx
        ↓
5b_validacion                 → verifica que el dinero cuadra · usa devoluciones para conciliación
        ↓
6_corte
        ↓
6b_override                   → correcciones de usuarios si las hay
        ↓
3_boletas
        ↓
7_cierre
...
│   ├── pagos_yape_tepago.xlsx   → resultado final → 5_cobranza
...
│   └── blancos_acumulados.xlsx  → lo lee 5_cobranza para DESCUENTO_YAPE_BLANCO
│
...
> motor_matching como 5_cobranza desde el mismo lugar.
...
**6. Pasar los outputs a 5_cobranza:**

```
outputs/pagos_yape_tepago.xlsx               → 5_cobranza
blancos/blancos_acumulados.xlsx              → 5_cobranza (vía shared/)
devoluciones/devoluciones_acumulados.xlsx    → 5_cobranza y 5b_validacion
```
...
| `Outputs/pagos_yape_tepago.xlsx` | Se borra | ya fue consumido por 5_cobranza |
...
Resultado final del matching. Solo lectura — generado automáticamente. Lo consume 5_cobranza.
...
| `identificado` | dueño encontrado · 5_cobranza aplica descuento |
...
Lo leen 5_cobranza para descuentos y 5b_validacion para conciliación.
...
|--------|-------------|--------------|
| `pendiente` | devolución a usuario · aún no descontada | 5_cobranza aplica descuento |
| `aplicado` | descuento ya aplicado | historial |
| `no aplica` | gasto JASS · no genera descuento | 5b_validacion lo suma como egreso en conciliación |
