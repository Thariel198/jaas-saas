...
```
lista_corte.xlsx  ←  5_cobranza
cobranza_final.xlsx  ←  5_cobranza        ↘
pagos_yape_tepago.xlsx  ← 4_pagos (re-corrido)  →  6_corte  →  pagaron_penalidad.xlsx
...
- [ ] `5_cobranza/main.py` ya corrió y generó `lista_corte.xlsx` y `cobranza_final.xlsx`
...
- [ ] `4_pagos` fue re-corrido con el nuevo reporte → `pagos_yape_tepago.xlsx` actualizado
...
Abrir una terminal en la carpeta `6_corte/` y ejecutar:
...
```
═══════════════════════════════════════════════════════
  6_corte — Clasificación penalidad y corte físico
═══════════════════════════════════════════════════════
...
PRE-CONDICIONES (en orden)
  5_cobranza corrió → lista_corte.xlsx + cobranza_final.xlsx
  Descargar nuevo reporte banco (período extendido)
  Re-correr 4_pagos → pagos_yape_tepago.xlsx actualizado
  Actualizar pagos_efectivo.xlsx si hubo cobros en mano

INPUTS (no copies nada — el módulo los lee directo)
  5_cobranza/outputs/lista_corte.xlsx
  5_cobranza/outputs/cobranza_final.xlsx
  5_cobranza/inputs/pagos_yape/pagos_yape_tepago.xlsx   ← actualizado
  5_cobranza/inputs/pagos_efectivo/pagos_efectivo.xlsx  ← actualizado
