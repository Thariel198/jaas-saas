# Manual de uso — 06a_enriquecimiento
...
config_mes.xlsx     ←  tú llenas una vez por mes
                  ...
                  06a_enriquecimiento
                            ↓
              06_boletas/inputs/data_boletas.xlsx
                            ↓
                       06_boletas
                            ↓
                    RECIBO_XXXXX.pdf × N
...
- [ ] `04_cobranza` ya corrió el mes anterior → los valores de ARRASTRE,
...
Abrir una terminal en la carpeta `06a_enriquecimiento/` y ejecutar:
...
```
═════════════════════════════════════════════════
  06a_enriquecimiento — Preparación DATA_boletas
═════════════════════════════════════════════════
...
═════════════════════════════════════════════════
  ...
  → data_boletas.xlsx
  Siguiente paso: correr 06_boletas/main.py
═════════════════════════════════════════════════
```
...
El archivo generado es `06_boletas/inputs/data_boletas.xlsx`, hoja `Data`.
...
Los PDFs quedarán en `06_boletas/Outputs/`.
...
INPUTS (no copiar — el módulo los lee directo)
  04_cobranza/inputs/planilla_base/planilla_base.xlsx
  inputs/config_mes.xlsx

OUTPUT
  06_boletas/inputs/data_boletas.xlsx   ← listo para 06_boletas

SIGUIENTE PASO
  06_boletas/main.py   → genera RECIBO_XXXXX.pdf por usuario
```
